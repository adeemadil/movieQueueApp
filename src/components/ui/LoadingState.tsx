...
const LoadingState = forwardRef<HTMLDivElement, LoadingStateProps>(
  (
    ...
    const skeletonBase =
      'bg-gradient-to-r from-neutral-800 via-neutral-700 to-neutral-800 rounded'

    // Skeleton components for different content types
    const SkeletonCard = ({ index = 0 }: { index?: number }) => (
      <motion.div
        className={cn(
          'bg-glass-primary rounded-xl p-6 space-y-4',
          shimmerClasses
        )}
    ...
    const SkeletonRecommendation = ({ index = 0 }: { index?: number }) => (
      <motion.div
        className={cn(
          'bg-glass-primary rounded-xl overflow-hidden',
          shimmerClasses
        )}
    ...
    const SkeletonContext = ({ index = 0 }: { index?: number }) => (
      <motion.div
        className={cn(
          'bg-gradient-to-br from-primary-900/30 to-primary-800/20 rounded-2xl p-8 text-center space-y-4',
          shimmerClasses
        )}
    ...
    const SkeletonList = ({ index = 0 }: { index?: number }) => (
      <motion.div
        className={cn(
          'flex items-center space-x-4 p-4 bg-glass-subtle rounded-lg',
          shimmerClasses
        )}
      ...
      >
        <div
          className={cn(skeletonBase, 'h-12 w-12 rounded-lg flex-shrink-0')}
        />
    ...
    const skeletonComponents = {
      ...
      list: SkeletonList,
      image: () => (
        <div className={cn(skeletonBase, 'h-32 w-full', shimmerClasses)} />
      ),
      text: () => (
    ...
    const SkeletonComponent =
      skeletonComponents[content] || skeletonComponents.card
